...
from biobb_ml.regression.common import check_input_path, check_output_path, getHeader, get_list_of_predictors, get_keys_of_predictors
    ...
    def launch(self) -> int:
        ...
        with open(self.io_dict["in"]["input_model_path"], "rb") as f:
            while True:
                try:
                    m = joblib.load(f)
                    if (isinstance(m, linear_model.LinearRegression) or isinstance(m, ensemble.RandomForestRegressor)):
                        new_model = m
        ...
        if 'poly_features' in locals():
            new_data = poly_features.transform(new_data)
