...
"""Module containing the KMeansTraining class and the command line interface."""
import argparse
import io
...
class KMeansTraining():
    ...
    Args:
        ...
        output_wcss_path (str): Path to the inertia values list. Accepted formats: csv.
        output_gap_path (str): Path to the gap values list. Accepted formats: csv.
    ...
    def __init__(self, input_dataset_path,
                 output_wcss_path, output_gap_path, output_plot_path=None, properties=None, **kwargs) -> None:
        properties = properties or {}

        # Input/Output files
        self.io_dict = { 
            "in": { "input_dataset_path": input_dataset_path }, 
            "out": { "output_wcss_path": output_wcss_path, "output_gap_path": output_gap_path, "output_plot_path": output_plot_path } 
        }
    ...
    def check_data_params(self, out_log, err_log):
        ...
        self.io_dict["out"]["output_wcss_path"] = check_output_path(self.io_dict["out"]["output_wcss_path"],"output_wcss_path", False, out_log, self.__class__.__name__)
        self.io_dict["out"]["output_gap_path"] = check_output_path(self.io_dict["out"]["output_gap_path"],"output_gap_path", False, out_log, self.__class__.__name__)
        self.io_dict["out"]["output_plot_path"] = check_output_path(self.io_dict["out"]["output_plot_path"],"output_plot_path", True, out_log, self.__class__.__name__)

    @launchlogger
    def launch(self) -> int:
        """Launches the execution of the KMeansTraining module."""
        ...
        if self.restart:
            output_file_list = [self.io_dict["out"]["output_wcss_path"],self.io_dict["out"]["output_gap_path"],self.io_dict["out"]["output_plot_path"]]
        ...
        # save wcss table
        fu.log('Saving WCSS table to %s' % self.io_dict["out"]["output_wcss_path"], out_log, self.global_log)
        wcss_table.to_csv(self.io_dict["out"]["output_wcss_path"], index = False, header=True, float_format='%.3f')
        ...
        # save gap table
        fu.log('Saving Gap results to %s' % self.io_dict["out"]["output_gap_path"], out_log, self.global_log)
        gap_table.to_csv(self.io_dict["out"]["output_gap_path"], index = False, header=True, float_format='%.3f')
...
def main():
    ...
    required_args.add_argument('--output_wcss_path', required=True, help='Path to the inertia values list. Accepted formats: csv.')
    required_args.add_argument('--output_gap_path', required=True, help='Path to the gap values list. Accepted formats: csv.')
    ...
    KMeansTraining(input_dataset_path=args.input_dataset_path,
                   output_wcss_path=args.output_wcss_path, 
                   output_gap_path=args.output_gap_path, 
