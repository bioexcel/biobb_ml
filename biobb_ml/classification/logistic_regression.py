...
class LogisticRegression(BiobbObject):
    """
    ...
    def launch(self) -> int:
        ...
        # classification
        fu.log('Training dataset applying logistic regression', self.out_log, self.global_log)
        model = linear_model.LogisticRegression(C = self.c_parameter, solver = self.solver, random_state = self.random_state_method)
